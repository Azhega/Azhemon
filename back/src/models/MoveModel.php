...
class MoveModel extends SqlConnect {
  ...
  public function add(array $data) {
    ...
    if ($req->rowCount() > 0) {
      throw new HttpException("Move already exists!", 400);
    }
  ...
  public function getAll(?int $limit = null) {
    ...
    if ($req->rowCount() == 0) {
      throw new HttpException("No Moves !", 400);
    }
  ...
  public function delete(int $id) {
    ...
    $deletedMoveName = $res['name'];

    $req = $this->db->prepare("DELETE FROM $this->table WHERE id = :id");
    $req->execute(["id" => $id]);
    return [
      'message' => 'Move ' . $deletedMoveName . ' successfully removed !',
    ];
  }
}