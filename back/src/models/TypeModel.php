...
class TypeModel extends SqlConnect {
  private $table = "type";
  ...
  public function get(int $id) {
    ...
    if ($req->rowCount() == 0) {
      throw new HttpException("Type doesn't exists !", 400);
    }
  ...
  public function getAll(?int $limit = null) {
    ...
    if ($req->rowCount() == 0) {
      throw new HttpException("No types !", 400);
    }
  ...
  public function delete(int $id) {
    ...
    $deletedTypeName = $res['name'];

    $req = $this->db->prepare("DELETE FROM $this->table WHERE id = :id");
    $req->execute(["id" => $id]);
    return [
      'message' => 'Type ' . $deletedTypeName . ' successfully removed !',
    ];
  }
}